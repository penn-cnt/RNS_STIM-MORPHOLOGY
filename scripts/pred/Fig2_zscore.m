...
colors = ['g','b'];
% max_time_length = max(cellfun(@(x) size(x,2),zplv_traces),[],'all');
        ...
        for i = 1:2 % conntype
            for j = 1:4 % freq
                ...
                for n = 1:length(data)
                    ...
                    data(n).reorg_zplv = data(n).reorg_zplv{i+2,j};
                end
                for o = 1:2 % outcome
                    tmp = data(cellfun(@(x,y) x(y)==o,{data.outcome_group},tmp_year));
                    traces = horzcat(tmp.reorg_zplv)';
                    if ~isempty(traces)
...
for y = 1:length(years)
    for d = 1:2
        ...
        [zplv,outcome] = getYearOutcome(data,years{y},'outcome_group','reorg_zplv');
        f = figure(d);
        f.Position = [100,100,1200,600];
        for i = 1:2 % conntype
            for j = 1:4 % freq
                subplot(2,4,sub2ind([4,2],j,i))
                hold on
                plv_traces = cellfun(@(x) x{i,j},zplv,'UniformOutput',false);
                for o = 1:2 % outcome
                ...
                text(ttdays-200,-ylims(i)+0.1,['p=',num2str(fda_pv,'%.3f')],'FontSize',12);
                if ~isempty(sig_days)
                    scatter(time(sig_days(:,1)),zeros(size(sig_days,1),1),[],'r','filled');
                end
                if i == 1
        ...
        h.XLabel.Visible='on';
        h.YLabel.Visible='on';
        ylabel(h,'% zplv','Position',[-0.05,0.500000476837158,0], ...
                'FontWeight','bold','FontSize',14);
        xlabel(h,'Days','FontWeight','bold','FontSize',14);
        
        saveas(f,fullfile(figpath,'00_Network_trajectory','zplv',[depth_strings{d},'_',num2str(bin_size),'_',num2str(years{y}),'_lead.png']))
    end
    close all
end
%%
...
for i = 1:4 % conntype
    for j = 1:4 % freq
        ...
        hold on
        for o = 1:2 % outcome
            trace = [zplv_traces{1,i,o,j};zplv_traces{2,i,o,j}];
            if ~isempty(trace)
...
% if plot_figure
%     % get plv instead of zplv time trace
%
%
% end
% if plot_figure
...
%         for j = 1:size(zplv,3) % each frequency
%             subplot(7,1,i)
%             hold on
%             plot(binned_times(logical(binned_times)),binned_zplvs(:,i,j))
