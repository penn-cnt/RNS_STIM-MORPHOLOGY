...
%% Settings
function Fig2(bin_size,ttdays,regress_dist,outcome_option,option,years)
% clear; close all;
paths;
...
if ~exist(fullfile(figpath,'00_Network_trajectory',num2str(outcome_option),option(7:end)),'dir')
    mkdir(fullfile(figpath,'00_Network_trajectory',num2str(outcome_option),option(7:end)));
end
    ...
    for d = 1:2
        ...
        for i = 1:2 % conntype
            for j = 1:4 % freq
                ...
                text(ttdays-200,-ylims(i)+addp,['p=',num2str(fda_pv,'%.3f')],'FontSize',12);
                if ~isempty(sig_days)
...
%         figure(d)
        ...
        h.XLabel.Visible='on';
        h.YLabel.Visible='on';
        ylabel(h,option(7:end),'Position',[-0.05,0.500000476837158,0], ...
                'FontWeight','bold','FontSize',14);
        xlabel(h,'Days','FontWeight','bold','FontSize',14);
        
        saveas(f,fullfile(figpath,'00_Network_trajectory',num2str(outcome_option),option(7:end),[depth_strings{d},'_',num2str(bin_size),'_',num2str(years{y}),'_lead.png']))
    end
    close all
end
end
