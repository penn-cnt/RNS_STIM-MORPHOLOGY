function [plv,scales] = clip_filtered_plv(eeg, fs, freq_bands,options)
...
for conn = 1:num_connections
    ...
    [wave1,f1] = cwt(signal1,'amor',fs,'VoicesPerOctave',vpo,'FrequencyLimits',[4,100]);
    [wave2,~] = cwt(signal2,'amor',fs,'VoicesPerOctave',vpo,'FrequencyLimits',[4,100]);
    for freq = 1:length(f1)
...
%         plv(conn,freq) = def_plv(fsignal1,fsignal2);
        plv(conn,freq) = def_plv(wave1(freq,:),wave2(freq,:));
    end
end
...
function val = def_plv(signal1,signal2)
    ...
    val = abs(signal1*conj(signal2)')/length(signal1);
end
end
