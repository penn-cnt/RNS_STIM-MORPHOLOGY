...
% network_trajectory;
% %% bar plot
% outcomes = {'p_time','p_stim','p_time_bin','pCoeff_time','pCoeff_stim','pCoeff_time_bin',...
%     'p_time_noint','p_stim_noint','p_time_bin_noint','pCoeff_time_noint',...
%     'pCoeff_stim_noint','pCoeff_time_bin_noint'};
% for m = [1,5]
...
outcomes2 = {'PLV_std_bin','dPLV_std_bin','zPLV_std_bin',...
    ...
    'R2_time_noint','p_time_noint','Coeff_time_noint',...
    'R2_stim_noint','p_stim_noint','Coeff_stim_noint',...
    'R2_time_bin_noint','p_time_bin_noint','Coeff_time_bin_noint',...
    'pCoeff_time','pCoeff_time_bin','pCoeff_stim',...
    'pCoeff_time_noint','pCoeff_time_bin_noint','pCoeff_stim_noint'};
for m = [5]
    for i = [12,18]%length(outcomes2)
        results = var_conn(90,1050,outcomes2{i},m);
        names = results.Properties.VariableNames;
        results.Properties.VariableNames = strcat(repmat({['M',num2str(m)]},1,8),'_',names);
        writetable(results,['user_data/stats/',outcomes2{i},'_M',num2str(m),'_log.csv'],'WriteRowNames',true);
    end
end
