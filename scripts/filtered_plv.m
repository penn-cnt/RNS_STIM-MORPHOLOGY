...
plv = zeros(size(eeg,3),num_connections,1);
for seg = 1:size(eeg,3)
    for conn = 1:num_connections
        for freq = 1:num_freqs
            signal1 = squeeze(eeg(connections(conn,1),:,seg));
            signal2 = squeeze(eeg(connections(conn,2),:,seg));
            ...
            plv(seg,conn,freq) = def_plv(fsignal1,fsignal2);
        end
    end
end
    ...
    function val = def_plv(signal1,signal2)
        ...
        val = abs(sum(signal1*conj(signal2)'))/length(signal1);
    end
