...
basepath = rns_config.paths.base;
rootpath = fullfile(basepath,rns_config.paths.RNS_project_code_path);
rns_toolbox_path = fullfile(basepath,rns_config.paths.RNS_processing_toolbox);
addpath(genpath(rootpath))
addpath(genpath(rns_toolbox_path))
datapath = fullfile(basepath,rns_config.paths.RNS_PROCESSED_DATA_Folder);
rns_config_path = fullfile(datapath, 'config.JSON');
...
ptList = patient_info.ID; % H: changed to use more updated?
rng(88)
%%
for pt = 1:length(ptList)
    ...
    if isempty(con_labels)
        localization(pt).meets_criteria = false; % H:I think this is never empty
    end
