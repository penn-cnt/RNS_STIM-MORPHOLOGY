...
freqs = all_plvs(1,1,:); % Only if cwt
all_plvs = all_plvs(:,2:end,:); % Only if cwt
...
cum_stims = movingCumulativeSum(stim_data{:,"EpisodeStarts"},180*24);
...
mdl = fitlme(data_table,"DPLV ~ Baseline*Connection + Baseline*Depth + (1|IDs)")
...
plasticity = load(fullfile(datapath,sprintf("corr_plasticity_%s.mat",bin_size))).plasticity;
